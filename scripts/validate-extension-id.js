...
try {
  ...
  const xmlPath = path.join(__dirname, "..", "config", "manifest.xml");
  ...
  if (process.argv.length > 2) {
    ...
    console.log(
      `   Match with our calculation: ${
        chromeId === calculatedId ? "YES ✅" : "NO ❌"
      }`
    );
    ...
    if (chromeId !== calculatedId) {
      ...
      console.log(
        "   This may cause issues with extension updates and consistency."
      );
    }
  }
} catch (error) {
