...
console.log('🔧 Setting up Git hooks for Chrome Extension Template maintainers...\n');

// Warning for end users
console.log('⚠️  WARNING: This setup is for TEMPLATE MAINTAINERS ONLY!');
console.log('   If you are developing your own extension, DO NOT run this script.');
console.log('   This will reset your extension to template defaults before every commit.\n');
...
rl.question('Are you a template maintainer setting up hooks? (yes/no): ', (answer) => {
  if (answer.toLowerCase() !== 'yes' && answer.toLowerCase() !== 'y') {
    console.log('❌ Setup cancelled. This is the correct choice for extension developers.');
    process.exit(0);
  }
  ...
  try {
    ...
    if (process.platform !== 'win32') {
      ...
      try {
        ...
        console.log('⚠️  Could not make hook executable. You may need to run: chmod +x .githooks/pre-commit');
      }
    }
    ...
    const resetScript = path.join(projectRoot, 'maintainer', 'reset-extension.js');
    ...
    console.log('4. The reset script will run automatically before the commit');
